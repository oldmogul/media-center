...
(function () {
    function init() {
        ...
        function formatBody(raw) {
            ...
            function flushShort() {
                ...
                if (shortRun.length === 1)
                    out.push(`<p>${esc(shortRun[0])}</p>`);
                else
                    out.push(`<ul>${shortRun.map((i) => `<li>${esc(i.replace(/^[-*•]\s+/, "").replace(/^\d+[\.)]\s+/, ""))}</li>`).join("")}</ul>`);
                shortRun = [];
            }
            for (const lines of blocks) {
                ...
                if (/^FOR GOD AND MY COUNTRY/i.test(first)) {
                    ...
                    if (lines.length > 1)
                        out.push(`<p class="rel-sig">${lines.slice(1).map(esc).join("<br>")}</p>`);
                    continue;
                }
                ...
                const looksList = lines.length >= 2 && short && lines.every((l) => /^[-*•]/.test(l) || /^\d+[\.)]/.test(l) || l.length < 110);
                if (looksList) {
                    flushShort();
      ...
      <p class="hero-actions"><a class="btn-gold" href="news.html">${esc(t("press.back") || "Back to Press Room")}</a></p>
        ...
        if (desc)
            desc.setAttribute("content", (item.p || item.title).slice(0, 180));
