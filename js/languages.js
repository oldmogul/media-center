function init() {
      ...
      <span>${l.native}</span>
        ...
        box.querySelectorAll("[data-lang]").forEach((btn) => {
            btn.addEventListener("click", () => {
                const to = document.getElementById("lang-to");
                if (to)
                    to.value = btn.getAttribute("data-lang");
    ...
    function engineCode(lang) {
        if (!lang)
            return "en";
        if (lang.id === "en" || lang.id === "sw" || lang.id === "lg")
            return lang.code;
        if (lang.code === "rw" || lang.code === "ar")
            return lang.code;
        return "sw";
    }
    async function translateText(text, fromId, toId) {
        ...
        const run = async (a, b) => {
            try {
                return await googleTranslate(text, a, b);
            }
            catch (e) {
                return await memoryTranslate(text, a === "auto" ? "en" : a, b);
            }
        };
        try {
    ...
    function init() {
        if (document.body.getAttribute("data-page") !== "languages")
            return;
        fillSelects();
        renderMosaic("all");
        const n = document.querySelector(".lang-strip-count b");
        if (n)
            n.textContent = String(LANGS.length);
