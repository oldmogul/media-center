...
(function () {
    ...
    function mount(rootPath) {
        if (document.getElementById("ugov-widget"))
            return;
        const L = lang();
        const u = UI[L];
      ...
      <button type="button" class="ugov-fab" id="ugov-fab" aria-controls="ugov-dock" aria-expanded="false" aria-label="${u.title}">
        ...
        const input = document.getElementById("ugov-q");
        ...
        function openDock(open) {
            ...
            if (open && thread && !thread.childNodes.length)
                greet();
            if (open)
                input?.focus();
        }
