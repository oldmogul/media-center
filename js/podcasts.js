    ...
    function init() {
        ...
        const SCHEDULE = (function () {
            ...
            for (let i = 1; i <= 7; i++) {
                const d = i === 7 ? 0 : i;
                if (d !== 0)
                    rows.push({ day: d, time: "05:30", show: "farmers" });
                rows.push({ day: d, time: "07:00", show: "bulletin" });
                if (d === 0)
                    rows.push({ day: d, time: "08:00", show: "faith" });
                if (d === 6)
                    rows.push({ day: d, time: "09:00", show: "pdm" });
                if (d === 2 || d === 4)
                    rows.push({ day: d, time: "10:00", show: "health" });
                rows.push({ day: d, time: "13:00", show: "bulletin" });
                if (d === 1 || d === 5)
                    rows.push({ day: d, time: "16:00", show: "youth" });
                rows.push({ day: d, time: "18:00", show: "kiswahili" });
                rows.push({ day: d, time: "19:00", show: "bulletin" });
                if (d === 3)
                    rows.push({ day: d, time: "20:00", show: "citizen" });
            }
            return rows;
        })();
        const BED = [
        ...
        function upcomingSlots(n) {
            ...
            for (let add = 0; add < 8 && out.length < n; add++) {
                ...
                SCHEDULE.filter((s) => s.day === day).forEach((s) => {
                    if (out.length >= n)
                        return;
                    if (add === 0 && timeToMin(s.time) <= curMin)
                        return;
                    out.push(s);
                });
            }
        ...
        audio.addEventListener("timeupdate", () => {
            ...
            if (tEl)
                tEl.textContent = fmtTime(audio.currentTime) + " / " + fmtTime(audio.duration || 0);
            setPlayLabel(document.querySelector("[data-dock-play]"), audio.paused);
            if (now && now.live)
                setPlayLabel(document.getElementById("live-listen"), audio.paused);
        });
        ...
        function paintPublic() {
            ...
            if (art)
                art.src = on ? (live.cover || show.cover) : (show.cover || live.cover);
            const badge = stage.querySelector("[data-live-badge]");
            if (badge)
                badge.innerHTML = on ? "<i></i> Live press briefing" : "<i></i> Off-air";
        ...
        function renderSchedule() {
            ...
            const rows = SCHEDULE.filter((s) => {
                const sh = showById(s.show);
                if (day !== "all" && String(s.day) !== String(day))
                    return false;
                if (cat !== "all" && sh.cat !== cat)
                    return false;
                if (lang !== "all" && sh.lang !== lang)
                    return false;
                return true;
            });
      ...
      <div class="radio-show-actions">
        ...
        function initPublic() {
            ...
            window.addEventListener("storage", (ev) => { if (ev.key === KEY)
                paintPublic(); });
        ...
        function initAdmin() {
            ...
            if (epShow)
                epShow.innerHTML = SHOWS.map((s) => `<option value="${s.id}">${s.name}</option>`).join("");
            ...
            document.getElementById("reset-lib")?.addEventListener("click", () => {
                if (!confirm("Reset the library to the original UMC seed?"))
                    return;
        ...
        function paintAdmin() {
            ...
            if (live) {
                ...
                if (el("live-show"))
                    el("live-show").value = live.show;
                if (el("live-title"))
                    el("live-title").value = live.title;
                if (el("live-host"))
                    el("live-host").value = live.host || "";
                if (el("live-blurb"))
                    el("live-blurb").value = live.blurb || "";
                if (el("live-lang"))
                    el("live-lang").value = live.lang || "English";
                if (el("live-audio"))
                    el("live-audio").value = live.audio || "";
            }
