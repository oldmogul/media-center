(function () {
  ...
  const events = window.UMC_EVENTS || [];
  ...
  function webHref(web) {
    if (!web) return "#";
    if (web.startsWith("http") || web.includes(".html")) return web;
    return root + "/" + web;
  }
  function webLabel(web) {
    if (!web) return "";
    if (web === "languages.html") return t("nav.languages") || "Languages Desk";
